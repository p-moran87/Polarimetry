...
zimg = z[np.lexsort((X, Y))].reshape(Xu.size, Yu.size)

plt.imsave('polar_flux_map.png', zimg)
img = np.asarray(Image.open('polar_flux_map.png'))
lum_img = img[:, :, 0]
plt.imshow(lum_img)
plt.show()